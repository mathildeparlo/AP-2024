...
eval env (Lambda vname e) = Right $ ValFun env vname e

eval env (Apply fun arg) =
  case eval env fun of
    Left err -> Left err
    Right (ValFun funEnv vname body) ->
      case eval env arg of
        Left err -> Left err
        Right val -> eval (envExtend vname val funEnv) body
    Right _ -> Left "Non-function application"
